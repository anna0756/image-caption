...
def resnet18(**kwargs):
    return torchvision.models.resnet18(**kwargs)
...
if __name__ == "__main__":
    ...
    from dataset.image_caption import ImageCaption, collate_func

    tokenizer = basic_tokenizer(os.path.join(CURRENT_PATH, '../data/basic_vocab.txt'))
    dataset = ImageCaption(os.path.join(CURRENT_PATH, '../data/LCCC_base/test'), tokenizer)
    batch_size = 10
    ...
    for batch_idx, batch in tqdm(enumerate(train_loader), total=int(len(train_loader.dataset) / batch_size) + 1):
        src_batch, tgt_batch, src_pad_batch, tgt_pad_batch, tgt_mask_batch = batch['src_ids'], batch['tgt_ids'], batch[
            'src_pad_mask'], batch['tgt_pad_mask'], batch['tgt_mask']
        break
    ##
    model = transformer_base(tokenizer.vocab_size, 300, 15, os.path.join(CURRENT_PATH,
                                                                         '../data/pretrained_embed.pkl'))  # ,os.path.join(CURRENT_PATH,'../data/pretrained_embed.pkl')
